...
let { baseURL, tokenName, strongToken, contentType, withCredentials, responseType } = configObj;
...
axios.defaults.validateStatus = function (status) {
  return (status >= 200 && status < 300) || status === 460 || status === 461;
};
...
axios.interceptors.response.use((res) => {
    if (res.status === 460) {
        const authorization = res.headers && res.headers['authorization'];
        if (authorization) {
            const newToken = String(authorization).replace(/^Bearer\s+/i, '').trim();
            if (newToken) {
                const userStore = useUserStore();
                userStore.token = newToken;
                setStorage(JSON.stringify(newToken), strongToken);
            }
        }
    }
    if (res.status === 461) {
