<template>
	<div class="user">
		...
		<div class="common-seach-wrap">
			<el-form size="small" :inline="true" :model="searchForm" class="demo-form-inline">
				<el-form-item label="订单号">
					<el-input size="small" v-model="searchForm.orderNo" placeholder="请输入订单号"></el-input>
				</el-form-item>
				<el-form-item label="配送方式">
					<el-select size="small" v-model="searchForm.styleId" placeholder="请选择">
				...
				<el-form-item label="起始时间">
					<div class="block">
						...
						<el-date-picker size="small" v-model="searchForm.createTime" type="daterange"
			...
			<div class="table-wrap">
				...
				<el-table size="small" :data="tableData.data" :span-method="arraySpanMethod" border style="width: 100%"
					...
					<el-table-column prop="deliveryType" label="配送方式">
						<template #default="scope">
							<div v-if="!scope.row.isTopRow">
								...
								<span v-if="scope.row.deliveryType == 30">手机号:{{ scope.row.mobile }}</span>
							</div>
						</template>
					</el-table-column>
                  ...
                  " @click="verifyClick(scope.row)" type="text" size="small" v-auth="'/takeout/operate/extract'">确认送达
								...
								<el-button @click="senDada(scope.row)" v-if="
                    scope.row.payStatus == 20 &&
                    scope.row.deliverStatus == 0 &&
                    scope.row.orderStatus == 10 &&
                    scope.row.deliveryType == 10
                  " type="text" size="small" v-auth="'/takeout/operate/sendOrder'">{{ deliverName.name }}
								</el-button>
								<el-button v-if="
		...
		<Cancel v-if="open_edit" :open_edit="open_edit" :orderNo="orderNo" :orderId="orderId"
			@closeDialog="closeDialogFunc($event, 'edit')">
		</Cancel>
		<!--处理-->
		<refund v-if="open_refund" :open_edit="open_refund" :orderNo="orderNo" :orderId="orderId"
	...
	export default {
		...
		methods: {
			...
			onExport: function() {
				...
				Params.pageIndex = self.curPage;
				Params.pageSize = self.pageSize;
        request._download('/shop/takeout/operate/export', Params, true).then(res => {
          let disposition = res.headers && (res.headers['content-disposition'] || res.headers['Content-Disposition']);
          let filename = 'order.xlsx';
          if (disposition) {
            let match = /filename\*=UTF-8''([^;]+)|filename="?([^\";]+)"?/i.exec(disposition);
            let raw = (match && (match[1] || match[2])) || '';
            if (raw) {
