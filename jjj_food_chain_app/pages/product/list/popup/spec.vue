		...
		methods: {
			...
			addCart() {
				...
				self.form.showSku.feed.forEach((item, index) => {
					if (item != null) {
						feed.push(self.detail.productFeed[item].feedName);
					}
				});
				if (feed.length <= 0) {
					feed = "";
				...
				let attr = [];
				self.form.showSku.attr.forEach((item, index) => {
					if (item != null) {
						attr.push(self.detail.productAttrList[index].attributeValue[item]);
					}
				});
				if (attr.length <= 0) {
					attr = "";
