...
public class UserWxController extends BaseController {
    ...
    public ApiResult<LoginUserTokenVo> userLogin(@RequestBody AppWxParam appWxParam){
        ...
        // 设置token响应头
        response.setHeader(JwtTokenUtil.getTokenName(""), JwtTokenUtil.buildAuthorizationValue(user.getToken()));
    ...
    public ApiResult<LoginUserTokenVo> bindMobile(String code, String encryptedData, String iv){
        User currentUser = this.getUser(true);
        LoginUserTokenVo user = userService.bindMobileByWx(currentUser.getUserId(), code, encryptedData, iv);
        // 设置token响应头
        response.setHeader(JwtTokenUtil.getTokenName(""), user.getToken());
        return ApiResult.ok(user, "登录成功");
    }
