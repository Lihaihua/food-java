...
public class UserCartServiceImpl extends BaseServiceImpl<UserCartMapper, UserCart> implements UserCartService {
    ...
    public Boolean add(User user, UserCartParam param) {
        ...
        if (userCart != null) {
            ...
            //包装费
            saveBean.setBagPrice(productSku.getBagPrice());
            //商品价格
            saveBean.setProductPrice(productSku.getProductPrice());
            saveBean.setPrice(productSku.getProductPrice());
            //校验属性
            if(StringUtils.isNotBlank(param.getAttr())){
                if(StringUtils.isBlank(product.getProductAttr())){
                    throw new BusinessException("很抱歉，商品属性不存在");
                }
                List<String> attrList = Arrays.asList(param.getAttr().split(";"));
                List<AttrParam> productAttr = JSONArray.parseArray(product.getProductAttr(),AttrParam.class);
                for (String attrStr : attrList) {
                    List<AttrParam> selectAttr = productAttr.stream().filter(attrParam -> attrParam.getAttributeValue().contains(attrStr)).collect(Collectors.toList());
                    if(CollectionUtils.isEmpty(selectAttr)){
                        throw new BusinessException("很抱歉，商品属性不存在");
                    }
                }
            }
            //加料价格
            if(StringUtils.isNotBlank(param.getFeed())){
                if(StringUtils.isBlank(product.getProductFeed())){
                    throw new BusinessException("很抱歉，加料信息不存在");
                }
                List<String> feedList = Arrays.asList(param.getFeed().split(";"));
                List<FeedParam> productFeed = JSONArray.parseArray(product.getProductFeed(),FeedParam.class);
                for (String feedStr : feedList) {
                    List<FeedParam> selectFeed = productFeed.stream().filter(feedParam -> feedParam.getFeedName().equals(feedStr)).collect(Collectors.toList());
                    if(CollectionUtils.isEmpty(selectFeed)){
                        throw new BusinessException("很抱歉，加料信息不存在");
                    }
                    FeedParam feed = selectFeed.get(0);
                    saveBean.setProductPrice(saveBean.getProductPrice().add(feed.getPrice()));
                    saveBean.setPrice(saveBean.getPrice().add(feed.getPrice()));
                }
            }
            param.setAttr(StringUtils.isBlank(productSku.getSpecName()) ? param.getAttr() : productSku.getSpecName() + ";" + param.getAttr());
            saveBean.setDescrib(StringUtils.isBlank(param.getAttr()) ? param.getFeed() : param.getAttr() + ";" + param.getFeed());
     ...
     */
    @Override
    public Boolean sub(Integer userId, UserCartParam param) {
        UserCart cart = this.getById(param.getCartId());
        // 校验购物车是否属于当前用户
        if (cart == null || !cart.getUserId().equals(userId)) {
            throw new RuntimeException("购物车不存在或无权限操作");
        }
        //添加:up, 减少:down
        if("down".equals(param.getType())){
            if (cart.getProductNum().intValue() > 1) {
