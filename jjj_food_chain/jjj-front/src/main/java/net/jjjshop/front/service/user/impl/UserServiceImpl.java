...
public class UserServiceImpl extends BaseServiceImpl<UserMapper, User> implements UserService {
    ...
    public LoginUserTokenVo login(String mobile, String password) {
        ...
        if (!PasswordUtil.matches(password, user.getSalt(), user.getPassword())) {
    ...
    private LoginUserTokenVo getLoginUserTokenVo(User user) {
        ...
        // 获取数据库中保存的盐值，如果为空则生成并更新
        String salt = user.getSalt();
        if (StringUtils.isBlank(salt)) {
            salt = SaltUtil.generateSalt();
            User updateBean = new User();
            updateBean.setUserId(user.getUserId());
            updateBean.setSalt(salt);
            this.updateById(updateBean);
            user.setSalt(salt);
        }
        String newSalt = SaltUtil.getSalt(salt, jwtProperties);
    ...
    public LoginUserTokenVo loginWx(AppWxParam appWxParam) {
        ...
        if (user != null) {
            ...
            if(userInfo != null){
                ...
                // 生成盐值
                bean.setSalt(SaltUtil.generateSalt());
                this.save(bean);
                //修改用户默认昵称和头像
    ...
    public Boolean renew(String mobile, String code, String password) {
        ...
        String salt = StringUtils.isBlank(user.getSalt()) ? SaltUtil.generateSalt() : user.getSalt();
        updateUser.setSalt(salt);
        updateUser.setPassword(PasswordUtil.encrypt(password, salt));
    ...
    public void logout(HttpServletRequest request) throws Exception {
        ...
        // 删除Redis缓存信息
        userLoginRedisService.deleteLoginInfo(token, username);
