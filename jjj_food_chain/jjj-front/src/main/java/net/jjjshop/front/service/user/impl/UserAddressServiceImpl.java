...
public class UserAddressServiceImpl extends BaseServiceImpl<UserAddressMapper, UserAddress> implements UserAddressService {
    ...
    public Boolean defaultAddress(Integer addressId, User user) {
        // 先校验地址是否属于当前用户
        UserAddress existAddress = this.getOne(new LambdaQueryWrapper<UserAddress>()
                .eq(UserAddress::getAddressId, addressId)
                .eq(UserAddress::getUserId, user.getUserId()));
        if(existAddress == null){
            throw new BusinessException("地址不存在或无权访问");
        }
     ...
     * @return
     */
    public UserAddressVo detail(Integer addressId, Integer userId) {
     ...
     * @return
     */
    public Boolean edit(UserAddressParam userAddressParam, Integer userId) {
        // 先校验地址是否属于当前用户
        UserAddress existAddress = this.getOne(new LambdaQueryWrapper<UserAddress>()
                .eq(UserAddress::getAddressId, userAddressParam.getAddressId())
                .eq(UserAddress::getUserId, userId));
        if(existAddress == null){
            throw new BusinessException("地址不存在或无权访问");
        }
    ...
    public Boolean delById(Integer addressId, User user) {
        ...
        return this.removeById(existAddress);
    }
