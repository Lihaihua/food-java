...
public class OrderSettledUtils {
    ...
    public Map<String, Object> settlement(User user, List<ProductBuyVo> productList, OrderBuyParam orderBuyParam){
        ...
        // 订单配送费
        this.setOrderExpressPrice(orderBuyParam);
        // 计算订单商品的实际付款金额
     ...
     * 设置订单商品配送费
     */
    private void setOrderExpressPrice(OrderBuyParam orderBuyParam)
    {
        //就餐类型,10外卖配送,20外卖自提,30店内打包40店内堂食
        if(orderBuyParam.getDelivery() != null && orderBuyParam.getDelivery() == 10){
            Supplier supplier = supplierService.getById(orderBuyParam.getShopSupplierId());
            // 配送费
            BigDecimal shippingFee = supplier.getShippingFee();
            this.orderData.setExpressPrice(shippingFee);
        }
    }
    ...
    private void getOrderData(){
        ...
        if(this.user.getAddressId() > 0){
            this.orderData.setAddress(userAddressService.detail(this.user.getAddressId(), this.user.getUserId()));
        }
        // 配送费用
    ...
    private void validateOrderForm(OrderBuyParam orderBuyParam){
        if(orderBuyParam.getProductNum() != null && orderBuyParam.getProductNum() <= 0){
