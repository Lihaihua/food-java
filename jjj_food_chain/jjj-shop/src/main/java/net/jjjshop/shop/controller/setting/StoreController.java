...
public class StoreController {
    ...
    @OperationLog(name = "index")
    @ApiOperation(value = "index", response = String.class)
    public ApiResult<String> index(@Validated @RequestBody StoreVo storeVo) throws Exception{
        JSONObject jsonData = (JSONObject) JSONObject.toJSON(storeVo);
        settingUtils.saveShopSetting(SettingEnum.STORE.getKey(), jsonData);
        appService.saveName(storeVo.getName());
