...
public class OrderServiceImpl extends BaseServiceImpl<OrderMapper, Order> implements OrderService {
    ...
    /**
     * 获取所有订单（优化版本，批量查询关联数据）
     * @param orderPageParam
     * @return
     */
    private List<OrderVo> getListAllOptimized(OrderPageParam orderPageParam) {
        LambdaQueryWrapper<Order> wrapper = new LambdaQueryWrapper<>();
        wrapper = this.transWrapper(wrapper, orderPageParam);
        ...
        List<Integer> userIds = orderList.stream().map(Order::getUserId).distinct().collect(Collectors.toList());
        List<Integer> supplierIds = orderList.stream().map(Order::getShopSupplierId).distinct().collect(Collectors.toList());
        ...
        if (CollectionUtils.isNotEmpty(orderIds)) {
            List<OrderProduct> orderProducts = orderProductService.list(
                new LambdaQueryWrapper<OrderProduct>()
                    ...
                    .orderByAsc(OrderProduct::getOrderProductId)
            );
            orderProductMap = orderProducts.stream().collect(Collectors.groupingBy(OrderProduct::getOrderId));
        }

        // 批量查询商品编号
        List<Integer> productIds = orderProductMap.values().stream()
            .flatMap(List::stream)
            .map(OrderProduct::getProductId)
            .distinct()
            .collect(Collectors.toList());
        Map<Integer, String> productNoMap = new HashMap<>();
        if (CollectionUtils.isNotEmpty(productIds)) {
            productNoMap = productService.listByIds(productIds).stream()
                .collect(Collectors.toMap(
                    product -> product.getProductId(),
                    product -> product.getProductNo() == null ? "" : product.getProductNo()
                ));
        }

        // 批量查询订单地址
        Map<Integer, OrderAddress> orderAddressMap = new HashMap<>();
        List<OrderAddress> addresses = orderAddressService.list(
            new LambdaQueryWrapper<OrderAddress>().in(OrderAddress::getOrderId, orderIds)
        );
        if (CollectionUtils.isNotEmpty(addresses)) {
            orderAddressMap = addresses.stream().collect(Collectors.toMap(OrderAddress::getOrderId, a -> a));
        }

        // 批量查询自提信息
        Map<Integer, OrderExtract> extractMap = new HashMap<>();
        List<OrderExtract> extracts = extractService.list(
            new LambdaQueryWrapper<OrderExtract>().in(OrderExtract::getOrderId, orderIds)
        );
        if (CollectionUtils.isNotEmpty(extracts)) {
            extractMap = extracts.stream().collect(Collectors.toMap(OrderExtract::getOrderId, e -> e));
        }
        ...
        if (CollectionUtils.isNotEmpty(supplierIds)) {
            supplierMap = supplierService.listByIds(supplierIds).stream()
                .collect(Collectors.toMap(s -> (Integer)BeanUtil.getFieldValue(s, "shopSupplierId"), s -> s));
        }

        // 组装数据
        final Map<Integer, User> finalUserMap = userMap;
        final Map<Integer, List<OrderProduct>> finalOrderProductMap = orderProductMap;
        final Map<Integer, String> finalProductNoMap = productNoMap;
        final Map<Integer, OrderAddress> finalOrderAddressMap = orderAddressMap;
        final Map<Integer, OrderExtract> finalExtractMap = extractMap;
        final Map<Integer, Supplier> finalSupplierMap = supplierMap;
        ...
        return orderList.stream().map(order -> {
            ...
            // 设置用户信息
            vo.setUser(finalUserMap.get(vo.getUserId()));

            // 设置商品信息
            List<OrderProduct> products = finalOrderProductMap.get(vo.getOrderId());
            if (CollectionUtils.isNotEmpty(products)) {
                List<OrderProductVo> productVos = products.stream().map(e -> {
                    OrderProductVo productVo = new OrderProductVo();
                    BeanUtils.copyProperties(e, productVo);
                    productVo.setImagePath(uploadFileUtils.getFilePath(productVo.getImageId()));
                    productVo.setProductNo(finalProductNoMap.getOrDefault(productVo.getProductId(), ""));
                    return productVo;
                }).collect(Collectors.toList());
                vo.setProduct(productVos);
            } else {
                vo.setProduct(new ArrayList<>());
            }
            ...
            // 设置地址信息
            if (vo.getDeliveryType().intValue() == DeliveryTypeEnum.WAIMAI.getValue().intValue()) {
                OrderAddress orderAddress = finalOrderAddressMap.get(vo.getOrderId());
                if (orderAddress != null) {
                    OrderAddressVo orderAddressVo = new OrderAddressVo();
                    BeanUtils.copyProperties(orderAddress, orderAddressVo);
                    vo.setAddress(orderAddressVo);
            ...
            // 设置自提信息
            if (vo.getDeliveryType().intValue() == DeliveryTypeEnum.ZITI.getValue().intValue()) {
                OrderExtract extract = finalExtractMap.get(vo.getOrderId());
                vo.setExtract(extract);
            }

            // 设置供应商信息
            Supplier supplier = finalSupplierMap.get(vo.getShopSupplierId());
            vo.setSupplier(supplier);
            if (supplier != null) {
                vo.setSupplierName((String)BeanUtil.getFieldValue(supplier, "name"));
            }
    ...
    public void exportList(OrderPageParam orderPageParam, HttpServletResponse httpServletResponse) throws Exception {
        List<OrderVo> orderList = this.getListAllOptimized(orderPageParam);
        //构建数据
