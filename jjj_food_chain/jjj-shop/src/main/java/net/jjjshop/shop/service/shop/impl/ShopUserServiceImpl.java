...
public class ShopUserServiceImpl extends BaseServiceImpl<ShopUserMapper, ShopUser> implements ShopUserService {
    ...
    public LoginShopUserTokenVo login(String username, String password){
        ...
        if (!PasswordUtil.matches(password, shopUser.getSalt(), shopUser.getPassword())) {
            throw new AuthenticationException("用户名或密码错误");
        }
        if (!PasswordUtil.isBcryptHash(shopUser.getPassword())) {
            ...
            shopUser.setPassword(encryptPassword);
        }
    ...
    public Boolean renew(String oldpass, String password, String confirmPass){
        ...
        if (!PasswordUtil.matches(oldpass, shopUser.getSalt(), shopUser.getPassword())) {
    ...
    public void logout(HttpServletRequest request) throws Exception {
        ...
        // 删除Redis缓存信息
        shopLoginRedisService.deleteLoginInfo(token, username);
