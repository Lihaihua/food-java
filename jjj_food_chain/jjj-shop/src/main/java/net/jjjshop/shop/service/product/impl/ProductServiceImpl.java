...
public class ProductServiceImpl extends BaseServiceImpl<ProductMapper, Product> implements ProductService {
    ...
    private void setProductParams(ProductParam productParam, Product product){
        ...
        if(CollectionUtils.isNotEmpty(productFeed)){
            //判断feedName不能重复
            Set<String> feedNameSet = productFeed.stream().map(FeedParam::getFeedName).collect(Collectors.toSet());
            if(feedNameSet.size() != productFeed.size()){
                throw new BusinessException("加料名称不能重复");
            }
            for(FeedParam item : productFeed){
                //去掉特殊符号,P：标点字符,S：符号（比如数学符号、货币符号等）,Z：分隔符（比如空格、换行等）
                item.setFeedName(item.getFeedName()
                        .replaceAll("[\\pP\\pS\\pZ]", ""));
            }
