...
public class SettingUtils {
    ...
    public Boolean saveSetting(String key, JSONObject jsonData){
        ...
        // 删除缓存
        redisClient.getBucket(CommonRedisKey.getRedisKey(CommonRedisKey.SETTING_DATA, shopSupplierId,key)).delete();
        if(setting == null){
            // 新增
            bean.setSetKey(key);
    ...
    public JSONObject getShopSetting(String key, Long appId) {
        ...
        String cacheKey = CommonRedisKey.getRedisKey(CommonRedisKey.SETTING_DATA, appId, key);
        // 如果缓存中存在,则返回
        RBucket<JSONObject> bucket = redisClient.getBucket(cacheKey);
        JSONObject object = bucket.get();
        if(object != null){
    ...
    public Boolean saveShopSetting(String key, JSONObject jsonData){
        ...
        Setting bean = new Setting();
        // 删除缓存
        redisClient.getBucket(CommonRedisKey.getRedisKey(CommonRedisKey.SETTING_DATA, RequestDetailThreadLocal.getRequestDetail().getAppId(), key)).delete();
        if(setting == null){
            // 新增
            bean.setSetKey(key);
            bean.setDescription(SettingEnum.getDescriptionByKey(key));
