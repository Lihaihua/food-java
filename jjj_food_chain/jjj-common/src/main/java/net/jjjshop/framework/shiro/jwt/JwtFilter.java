...
public class JwtFilter extends AuthenticatingFilter {
    ...
    protected AuthenticationToken createToken(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
        ...
        if (jwtProperties.isRedisCheck() || jwtProperties.isSingleLogin()) {
            ...
            if(url.startsWith("/api/admin/")){
                ...
                // 如果token不存在，检查是否被踢下线
                if (!redisExpired && adminLoginRedisService.isKickedOut(token)) {
                    throw new AuthenticationException(ApiCode.KICKED_OUT.getMessage());
                }
            }else if(url.startsWith("/api/shop/")){
                redisExpired = shopLoginRedisService.exists(token);
                // 如果token不存在，检查是否被踢下线
                if (!redisExpired && shopLoginRedisService.isKickedOut(token)) {
                    throw new AuthenticationException(ApiCode.KICKED_OUT.getMessage());
                }
            }
            if (!redisExpired) {
        ...
        if (jwtProperties.isSaltCheck()) {
            if(url.startsWith("/api/admin/")) {
                ...
                expireSecond = jwtProperties.getBackExpireSecond();
                adminLoginRedisService.refreshLastActive(token);
            }else if(url.startsWith("/api/shop/")){
                salt = shopLoginRedisService.getSalt(username);
                expireSecond = jwtProperties.getBackExpireSecond();
                shopLoginRedisService.refreshLastActive(token);
            }
        } else {
            salt = jwtProperties.getSecret();
        }
        return JwtToken.build(token, username, salt, expireSecond);
    }
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        try {
            ...
            // 检查是否是被踢下线的异常
            if (e.getMessage() != null && e.getMessage().contains("被踢下线")) {
                log.error("账号被踢下线", e);
                HttpServletResponse httpServletResponse = WebUtils.toHttp(response);
                ApiResult<Boolean> apiResult = ApiResult.fail(ApiCode.KICKED_OUT);
                try {
                    HttpServletResponseUtil.printJson(httpServletResponse, apiResult);
                } catch (Exception ex) {
                    ex.printStackTrace();
