...
public class AdminUserServiceImpl extends BaseServiceImpl<AdminUserMapper, AdminUser> implements AdminUserService {
    ...
    public LoginAdminUserTokenVo login(String username, String password){
        ...
        if (!PasswordUtil.matches(password, adminUser.getSalt(), adminUser.getPassword())) {
            throw new AuthenticationException("用户名或密码错误");
        }
        if (!PasswordUtil.isBcryptHash(adminUser.getPassword())) {
    ...
    public void logout(HttpServletRequest request) throws Exception {
        ...
        // 删除Redis缓存信息
        adminLoginRedisService.deleteLoginInfo(token, username);
